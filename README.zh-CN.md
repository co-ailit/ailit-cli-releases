...
`ailit` 是一个面向 Zhihuiji Space 销售流程的 Go 命令行工具，可在终端中完成本地配置与接口连通性检查、客户/商品/结算账户搜索，以及通过交互式流程或 JSON 草稿创建销售单。CLI 本地名称和配置目录使用 `ailit` 品牌，但后端接口目标仍是 `space.zhihuiji.cn`。
